  ...
  if (!window.hasOwnProperty('AuthorBaseElement')) {
          ...
          (function () {
            ...
            if (missingDependencies.length > 0) {
              console.error(`[ERROR] <author-menu> Required dependenc${missingDependencies.length !== 1 ? 'ies' : 'y'} not found: ${missingDependencies.map(d => `<${d}>`).join(', ').replace(', ' + missingDependencies[missingDependencies.length - 1], ' and ' + missingDependencies[missingDependencies.length - 1])}`);
              missingDependencies.forEach((dep, i) => console.info(`${i+1}. <${dep}> is available at ${'https://github.com/author-elements/menu'.replace('menu', dep.replace('author-', ''))}`));
            }
          })();
...
//# sourceMappingURL=author-menu.js.map
